...
MAX_MODBUS_REGISTERS = 125  # Modbus TCP FC3/FC4 max registers per request
...
async def read_all_registers_at_once(
    ...
    """Read all register groups in a single Modbus transaction.

    Computes the overall span (min start_addr to max end_addr) across all groups,
    issues ONE read call, then extracts each parameter from the raw response.
    ...
    overall_start = min(g['start_addr'] for g in groups)
    overall_end = max(g.get('end_addr', g['start_addr'] + g['count']) for g in groups)
    total_count = overall_end - overall_start

    if total_count <= MAX_MODBUS_REGISTERS:
        # Single Modbus read covers all registers
        try:
            data = client.read_registers(
                unit_id=meter_id,
                function_code=function_code,
                start_address=overall_start,
                count=total_count,
                troubleshoot=troubleshoot
            )
            ...
            for group in groups:
                for param in group['params']:
                    try:
                        offset_bytes = (param['address'] - overall_start) * 2
                        size_bytes = param['size'] * 2
                        param_data = data[offset_bytes:offset_bytes + size_bytes]
            ...
            print(f"Error reading all registers at once: {e}")
            for group in groups:
                for param in group['params']:
                    results[param['name']] = None

    else:
        # Span exceeds Modbus limit - fall back to per-group reads
        if troubleshoot:
            print(f"Span {total_count} > {MAX_MODBUS_REGISTERS} regs, using per-group reads ({len(groups)} groups)")
        for group in groups:
            group_results = await read_register_group(
                client=client,
                meter_id=meter_id,
                function_code=function_code,
                group=group,
...
async def read_meter(meter: str, current_time: float, read_minute_params: bool = False) -> Dict[str, float]:
    ...
    try:
        ...
        # Read all registers in one Modbus transaction
        all_values = None
        for attempt in range(2):
            all_values = await read_all_registers_at_once(
                client=client,
            ...
            if all_values and any(v is not None for v in all_values.values()):
                break
            if attempt < 1:
        ...
        # Process results
        if all_values:
            for param_name, value in all_values.items():
