...
from .models import Meter, PowerReading, PowerReading1Min, EnergyReading, TariffRate, FuelAdjustment, ToUPeakHours, EfficiencyIncentiveTier
...
def get_readings_summary_sync():
    ...
    power_summary = {}
    for meter_name in redis_store.get_all_meter_names():
        latest = redis_store.get_latest_reading(meter_name)
        if latest:
            power_summary[meter_name] = {
                'meter_name': meter_name,
                'latest_power_timestamp': latest.get('timestamp'),
                'voltage': latest.get('voltage'),
                'current': latest.get('current'),
                'active_power': latest.get('active_power', 0),
...
def get_realtime_data_sync(meter_name):
    ...
    latest_power = redis_store.get_latest_reading(meter_name)
    ...
    if latest_power:
        ts = latest_power.get('timestamp')
        if ts:
            from datetime import datetime as dt_cls
            parsed = dt_cls.fromisoformat(ts.replace('Z', '+00:00'))
            local_time = convert_to_local_time(parsed)
            realtime_data.update({
                'timestamp': ts,
                'local_time': local_time.isoformat(),
            })
        realtime_data.update({
            'voltage': latest_power.get('voltage'),
            'current': latest_power.get('current'),
            'active_power': latest_power.get('active_power', 0),
            'apparent_power': latest_power.get('apparent_power', 0),
            'reactive_power': latest_power.get('reactive_power', 0),
            'power_factor': latest_power.get('power_factor'),
            'frequency': latest_power.get('frequency'),
        })
...
def get_timeseries_point_sync(meter_name):
    ...
    return {
        ...
        'active_power': latest.get('active_power', 0) or 0,
        'voltage': latest.get('voltage', 0) or 0,
        'current': latest.get('current', 0) or 0,
        'power_factor': latest.get('power_factor', 0) or 0,
        'frequency': latest.get('frequency', 0) or 0,
    }
...
def ingest_meter_data(request):
    ...
    if serializer.is_valid():
        ...
        # Write power fields to Redis for real-time serving
        power_params = {
            'Voltage': 'voltage', 'Current': 'current',
            'Active Power': 'active_power', 'Apparent Power': 'apparent_power',
            'Reactive Power': 'reactive_power', 'Power Factor': 'power_factor',
            'Frequency': 'frequency',
        }
        power_data = {}
        ...
        if power_data and meter_name:
            timestamp_unix = request.data.get('timestamp')
            ts_dt = timezone.make_aware(datetime.fromtimestamp(timestamp_unix))
            power_data['meter_name'] = meter_name
            redis_store.store_power_reading(meter_name, ts_dt.isoformat(), power_data)
...
def meter_readings_summary(request):
    # Get latest power readings from Redis
    power_summary = {}
    for meter_name in redis_store.get_all_meter_names():
        latest = redis_store.get_latest_reading(meter_name)
        if latest:
    ...
    for meter_name in all_meters:
        ...
        # Determine status from power timestamp
        latest_ts = meter_data.get('latest_power_timestamp')
        if latest_ts:
            from datetime import datetime as dt_cls
            parsed = dt_cls.fromisoformat(latest_ts.replace('Z', '+00:00'))
            time_diff = timezone.now() - parsed
            meter_data['status'] = 'online' if time_diff < timedelta(minutes=5) else 'offline'
            meter_data['latest_reading_time'] = parsed
        elif 'latest_energy_reading' in meter_data:
...
def meter_historical_data(request, meter_name):
    ...
    power_qs = PowerReading1Min.objects.filter(
        meter_name=meter_name,
        bucket__gte=start_time
    ).order_by('bucket').values(
        'bucket', 'meter_name', 'voltage', 'current', 'active_power',
        'apparent_power', 'reactive_power', 'power_factor', 'frequency',
        'sample_count'
    )
    power_data = [
        {**row, 'timestamp': row.pop('bucket')} for row in power_qs
    ]
...
def timeseries_data(request, meter_name):
    ...
    if minutes:
        ...
        start_time = timezone.now() - timedelta(minutes=minutes_val)
    elif hours:
        minutes_val = int(hours) * 60
        start_time = timezone.now() - timedelta(hours=int(hours))
    else:
        minutes_val = 24 * 60
        start_time = timezone.now() - timedelta(hours=24)

    # Serve from Redis for short windows (<=15 min)
    if minutes_val <= 15:
        readings = redis_store.get_recent_readings(meter_name, minutes=minutes_val)
        power_timeseries = [
            {
                'timestamp': r['timestamp'],
                'voltage': r.get('voltage'),
                'current': r.get('current'),
                'active_power': r.get('active_power'),
    ...
    # Longer windows from continuous aggregate
    power_qs = PowerReading1Min.objects.filter(
        meter_name=meter_name,
        bucket__gte=start_time
    ).order_by('bucket').values(
        'bucket', 'voltage', 'current', 'active_power',
        'power_factor', 'frequency'
    )
    # Map bucket -> timestamp for frontend compatibility
    power_data = [
        {**row, 'timestamp': row.pop('bucket')} for row in power_qs
    ]
...
def power_quality_data(request, meter_name):
    ...
    try:
        ...
        # Get 1-min averaged readings from continuous aggregate
        power_readings = PowerReading1Min.objects.filter(
            meter_name=meter_name,
            bucket__gte=start_time
        ).order_by('bucket')
        ...
        # Group 1-min rows into 30-minute intervals in local timezone
        aggregated_data = []
        current_readings = []
        interval_start = None

        for reading in power_readings:
            local_time = convert_to_local_time(reading.bucket)
            ...
            if interval_time != interval_start and current_readings:
                avg_data = {
                    ...
                    'active_power': sum(r.active_power or 0 for r in current_readings) / len(current_readings),
                    'apparent_power': sum(r.apparent_power or 0 for r in current_readings) / len(current_readings),
                    'reactive_power': sum(r.reactive_power or 0 for r in current_readings) / len(current_readings),
...
def calculate_efficiency_incentive(consumption_kwh):
    """Calculate energy efficiency incentive using single-tier approach.

    Returns tuple: (rebate_amount_rm, rate_sen_per_kwh)
    Finds the tier that contains the consumption and applies that tier's rate to entire amount.
    """
    ...
    for tier in tiers:
        ...
        if consumption_kwh >= min_kwh and consumption_kwh <= max_kwh:
            rate = float(tier.rebate_sen_per_kwh)
            rebate = consumption_kwh * rate / 100
            return (abs(rebate), rate)

    return (0.0, 0.0)  # No matching tier
...
def calculate_general_tariff_billing(consumption_kwh, billing_month):
    ...
    try:
        ...
        consumption_kwh = float(math.ceil(consumption_kwh))
        ...
        efficiency_incentive, efficiency_rate = calculate_efficiency_incentive(consumption_kwh)
        subtotal_after_incentive = float(subtotal - efficiency_incentive)

        # Calculate KWTB charge (1.6% of subtotal after incentive)
        kwtb_charge = float(subtotal_after_incentive * 0.016)
        ...
        return {
            ...
            'efficiency_rate_sen': round(efficiency_rate, 2),
...
def calculate_tou_billing(energy_readings, billing_month):
    ...
    try:
        ...
        peak_kwh_total = math.ceil(peak_kwh_total)
        offpeak_kwh = math.ceil(offpeak_kwh)
        ...
        efficiency_incentive, efficiency_rate = calculate_efficiency_incentive(total_kwh)

        # KWTB charge (1.6% of subtotal after incentive)
        kwtb_charge = float((subtotal - efficiency_incentive) * 0.016)
...
def billing_calculation(request, meter_name):
    ...
    try:
        ...
        return Response({
            ...
            'summary': {
                'total_consumption_kwh': math.ceil(total_consumption),
                'total_cost_rm': round(total_cost, 2),
                'avg_monthly_consumption_kwh': math.ceil(avg_consumption),
...
def export_data(request, meter_name):
    ...
    try:
        ...
        if data_type == 'power':
            # Use continuous aggregate with database-level timezone conversion
            queryset = PowerReading1Min.objects.filter(
                meter_name=meter_name,
                bucket__gte=start_time,
                bucket__lte=end_time
            ).extra(
                select={'local_timestamp': "to_char(bucket AT TIME ZONE 'Asia/Kuala_Lumpur', 'YYYY-MM-DD HH24:MI:SS')"}
            ).order_by('bucket')
