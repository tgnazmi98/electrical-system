...
class ReadingsConsumer(AsyncWebsocketConsumer):
    ...
    def get_initial_timeseries(self, meter_name):
        readings = redis_store.get_recent_readings(meter_name, minutes=15)
        return [
            {
                'timestamp': r.get('timestamp', ''),
                'voltage': r.get('voltage', 0) or 0,
                'current': r.get('current', 0) or 0,
                'active_power': r.get('active_power', 0) or 0,
                'power_factor': r.get('power_factor', 0) or 0,
...
class DeviceConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_meter_reading(self, data):
        ...
        # Store power data in Redis
        power_fields = {
            'meter_name': meter_name,
            'voltage': realtime_data['voltage'],
            'current': realtime_data['current'],
            'active_power': realtime_data['active_power'],
            'apparent_power': realtime_data['apparent_power'],
            'reactive_power': realtime_data['reactive_power'],
            'power_factor': realtime_data['power_factor'],
            'frequency': realtime_data['frequency'],
        }
        await database_sync_to_async(redis_store.store_power_reading)(
            meter_name, dt.isoformat(), power_fields
        )
